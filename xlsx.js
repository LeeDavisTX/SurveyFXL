...
const XLSX_HEADERS = {
  codes: ["Code", "Name", "Geometry", "Category", "Layer", "Point Layer", "Description", "In Surface"],
  attributes: ["Code", "Order", "Attribute Name", "Prompt", "Type", "Entry", "Min", "Max", "Decimals", "Default"],
  options: ["Code", "Attribute Name", "Order", "Option"],
};

const XLSX_TYPE_TO_TAG = { text: "StringAttribute", number: "DoubleAttribute", integer: "IntegerAttribute", menu: "ListAttribute", photo: "PhotoAttribute" };
...
function buildLibraryXlsx(codes) {
  ...
  for (const c of codes) {
    codesRows.push([
      c.code, c.name, c.geometry, c.category,
      c.layer || "0",
      c.geometry === "Point" ? "" : codePointLayer(c),
      c.description, c.includeInSurface ? "TRUE" : "FALSE",
    ]);
...
function xlsxEntryMethod(text) {
  const t = String(text || "").trim();
  if (/^required$/i.test(t)) return "Required";
  if (/^office\s*use\s*only$/i.test(t)) return "OfficeUseOnly";
  return "Optional";
}
...
function importXlsxCodes(wb, priorCodes) {
  ...
  for (let i = 1; i < codesRows.length; i++) {
    ...
    const entry = {
      ...
      /* Layer is a real column, so the sheet wins; the library being replaced
         is only the fallback for a sheet that predates the column. */
      layer: xlsxCellAt(cells, cIdx["layer"]) || (was ? was.layer : "0"),
      pointLabelStyle: was ? was.pointLabelStyle : "",
      extra: was ? was.extra.slice() : [],
      attributes: [],
    };
    const pointLayer = xlsxCellAt(cells, cIdx["point layer"]);
    if (geometry !== "Point" && pointLayer) {
      const at = entry.extra.findIndex(([n]) => n === "PointLayer");
      if (at === -1) entry.extra.push(["PointLayer", pointLayer]);
      else entry.extra[at][1] = pointLayer;
    }
    codes.push(entry);
  ...
  if (attrRows && attrRows.length > 1) {
    ...
    for (let i = 1; i < attrRows.length; i++) {
      ...
      const attr = {
        ...
        /* OfficeUseOnly is a real entry method, not a typo for Optional -
           coercing it away here would quietly change what the data
           collector asks the crew for. */
        entryMethod: xlsxEntryMethod(xlsxCellAt(cells, aIdx["entry"])),
        isLabelVisible: wasAttr ? wasAttr.isLabelVisible : true,
        hadLabelVisible: wasAttr ? wasAttr.hadLabelVisible : false,
        extra: wasAttr ? wasAttr.extra.slice() : [],
        /* A text attribute can have a default too, and the sheet has a
           column for it - reading it only for numbers threw it away. */
        defaultValue: xlsxCellAt(cells, aIdx["default"]),
      };
