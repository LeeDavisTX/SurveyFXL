...
const ATTR_TAGS = ["StringAttribute", "DoubleAttribute", "IntegerAttribute", "ListAttribute"];
const TYPE_LABEL = { StringAttribute: "Text", DoubleAttribute: "Number", IntegerAttribute: "Integer", ListAttribute: "Menu" };
...
class FxlDocument {
  ...
  _parse() {
    ...
    this.schemaVersion = root ? root.getAttribute("SchemaVersion") : null;
  ...
  _readAttr(el) {
    const a = {
      ...
      name: el.getAttribute("Name") || "",
      description: el.getAttribute("Description") || "",
      entryMethod: el.getAttribute("EntryMethod") === "Required" ? "Required" : "Optional",
  ...
  /* Rebuild only the FeatureDefinitions block; every other section of the
     original document (line styles, symbols, label styles, layers) is kept
     byte-for-byte. */
  serialize() {
  ...
  _writeDefs() {
    ...
    for (const c of this.codes) {
      ...
      const desc = c.description ? ` Description="${escXml(c.description)}"` : "";
      out += `    <${tag} Code="${escXml(c.code)}" Name="${escXml(c.name || c.code)}"${desc} Category="${escXml(c.category)}"` +
        ` IncludeInSurface="${c.includeInSurface ? "true" : "false"}" Color="${escXml(c.color)}"` +
        ` Layer="${escXml(c.layer)}" PointLabelStyle="${escXml(c.pointLabelStyle || "")}"${writeExtraAttrs(c.extra)}>\n`;
      out += "      <Attributes>\n";
      for (const a of c.attributes) out += this._writeAttr(a);
      out += "      </Attributes>\n";
      out += `    </${tag}>\n`;
    }
    return out;
  ...
  _writeAttr(a) {
    ...
    const head = `Name="${escXml(a.name)}"` + opt("Description", a.description) +
      ` EntryMethod="${a.entryMethod === "Required" ? "Required" : "Optional"}"` +
      ` IsLabelVisible="${a.isLabelVisible === false ? "false" : "true"}"` +
    ...
    if (a.tag === "DoubleAttribute" || a.tag === "IntegerAttribute") {
      return `        <${a.tag} ${head}` + opt("MinimumValue", a.minimumValue) + opt("MaximumValue", a.maximumValue) +
        opt("NumberOfDecimals", a.numberOfDecimals) + opt("DefaultValue", a.defaultValue) + " />\n";
    }
    return `        <${a.tag} ${head} />\n`;
  }
}
...
function looksLikeHeader(fields) {
  ...
  // Fall back: a header has no numeric coordinate in columns 2 and 3.
  const numeric2 = /^-?\d*\.?\d+$/.test(up[1] || "");
  const numeric3 = /^-?\d*\.?\d+$/.test(up[2] || "");
  return !numeric2 && !numeric3 && up.some((v) => /POINT|NORTH|EAST|ELEV|DESC|CODE|ATTR/.test(v));
}
...
function baseName(filename, fallback) {
  const n = (filename || fallback || "export").replace(/\.(fxl|xml|csv|txt)$/i, "");
...
function renderLibraryEmpty() {
  return h("div", { class: "empty" }, [
    ...
    loadFxlButton("Load FXL file", true),
  ]);
}

function loadFxlButton(label, primary) {
  return fileButton(label, ".fxl,.xml,.zip", primary, async (file) => {
    let xmlText = null;
...
function refreshDetail() {
  ...
  const top = h("div", { class: "detail-top" }, [
    ...
    h("label", { class: "field" }, [
      ...
      h("select", {
        class: "input",
        onchange: (e) => { code.geometry = e.target.value; code.tag = GEOMETRY_TAG[e.target.value]; refreshCodeList(); },
...
const SUGGESTED_CATEGORIES = ["GENERAL"];
...
function categorySelect(code) {
  const sel = h("select", {
    ...
    onchange: (e) => { code.category = e.target.value; markStale(); },
  }, [
...
function openCategoryManager() {
  ...
  const blankLine = h("p", { class: "modal-note cat-blank" });
  ...
  return h("div", { class: "empty" }, [
    ...
    h("div", { class: "steps" }, [
      h("div", { class: "step" }, [
        ...
        loadFxlButton(fxlOk ? "Replace" : "Load FXL"),
      ]),
      h("div", { class: "step" }, [
